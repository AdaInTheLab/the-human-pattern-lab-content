...
8. ~~**Deployment cutover.**~~ **Resolved 2026-04-18. Notebook is live at https://notebook.thehumanpatternlab.com.** Subdomain pivot chosen unanimously by the Skulk (Vesper, Sage, Koda, Luna) over replacement of the main site. Stack: Dreamhost VPS (same box as the main site and lab-api) + Apache vhost + Cloudflare in front (orange-cloud proxy, Full SSL, Let's Encrypt on origin). Existing `/lab-notes/*` URLs on the main site are **frozen in place** — live, resolvable, archived-not-deleted, no redirects. Deploy pipeline: GitHub Actions in `hpl-notebook` checks out three siblings (hpl-notebook + one-front-door + the-human-pattern-lab-content), runs `bun run build`, rsyncs `dist/` to the VPS webroot. Triggers: push-to-main in hpl-notebook, and `repository_dispatch` from the content repo (so Skulk writing auto-deploys without touching hpl-notebook). Full round-trip: ~90 seconds from push to live.
9. ~~**Burrow pages.**~~ **Resolved 2026-04-18.** Five burrows configured in `hpl-notebook/ofd.config.js` with ledes (Field Notes, Myths & Manifestos, Infrastructure, Letters, Hunts) and `includes_types` inference for trace → field-notes and myth|manifesto → myths-manifestos. OFD's aggregator emits `/burrows/` index + per-burrow pages as synthetic site-mode pages that run through the same habitability audit as hand-authored content. Entries can opt in explicitly via `burrow:` frontmatter (string or array), or inherit type-based inclusion. Empty burrows render with their lede and an empty-state panel. Four entries seeded into Infrastructure; further curation is Miso + Ada work, not mechanical tagging.
...
| 2026-04-18 | **Burrows shipped.** OFD v0.2 gained a burrow aggregator; five curated rooms in hpl-notebook (Field Notes, Myths & Manifestos, Infrastructure, Letters, Hunts) with ledes and `includes_types` inference. Auto-pulls trace → field-notes and myth\|manifesto → myths-manifestos; explicit `burrow:` frontmatter overrides. Also landed in the same round: filing footer on every entry ("By [Voice]" + "Filed in [Burrow]" chips), breadcrumbs on per-item aggregate pages, active-state nav section-match, RSS 2.0 feed at `/rss.xml` with a themed XSLT stylesheet for browser-readable rendering (feed readers get plain RSS). Resolves Open Question #4 (RSS to skulk.ai mirror). Four entries seeded into Infrastructure burrow. | Ada + scribe (implementation), Miso (theming), Luna (implied structure validated in practice) |
| 2026-04-18 | **Deployment pipeline live.** `notebook.thehumanpatternlab.com` serves a real build. Stack: GitHub Actions in hpl-notebook checks out three siblings, runs `bun run build`, rsyncs `dist/` to the Dreamhost VPS over SSH; Cloudflare proxies with Full SSL over a Let's Encrypt origin cert. Triggers: push-to-main of hpl-notebook, `repository_dispatch` from the content repo (Skulk writing auto-deploys), manual workflow_dispatch. Setup resolved a chain of quirks: MSYS path conversion in `gh secret set --body`, SSH key not initially installed on the VPS, Cloudflare Full SSL + no origin cert returning Dreamhost's "Site not found" default vhost, Let's Encrypt ↔ Cloudflare redirect-loop avoided by temporarily gray-clouding the record for cert provisioning. End-to-end round-trip: ~90 seconds from push to live. Team effort — Ada did every step that couldn't happen from GHA (keygen, VPS key install, Dreamhost panel, Cloudflare DNS, redirect-loop foresight, unproxy dance). | Ada (VPS/DNS/cert), scribe (GHA workflows, secret plumbing, diagnosis) |
